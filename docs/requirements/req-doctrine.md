...
Utilizando a mesma lógica e estrutura dos arquivscripts [step07-extract-notes-mistral.py](versions/development/poc-v-d33/core/step07-extract-notes-mistral.py), [step06-extract-legislation-mistral.py](versions/development/poc-v-d33/core/step06-extract-legislation-mistral.py) e [step09-extract-decision-details-mistral.py](versions/development/poc-v-d33/core/step09-extract-decision-details-mistral.py), crie um novo script para substituir o [step08-doctrine-legislation-ai.py](versions/development/poc-v-d33/core/step08-doctrine-legislation-ai.py).
...
**Descrição**: O script deve iniciar carregando a partir dos arquivos YAML de **configuração**, os dados necessárias para conectar ao banco de dados MongoDB e para utilizar a API Mistral, e os prompts e parametros que serão utilziados.
    ...
    * Estruturar o código, para que seja facilmente definido qual provider de IA e prompt serão utilizados na etapa de consulta à API Mistral.
        - Deve ser possível definir o provider de IA e prompt a serem utilizados, alterando apenas uma variável no início do script, e as demais informações, parametros e configurações serão carregadas do respectivo itens dos arquivos de configuração.  
            ...
            - Exemplo: definindo no script o prompt_id = "extract-doctrines-from-md", o sistema deverá carregar as respectivas configurações do prompt que serão utilizadas (apenas as que serão utilizadas) do arquivo prompts.yaml: role, content, template_variables, client_parameters, etc.   
...
2. Consultar a collection case_data no banco de dados MongoDB, buscando o documento onde o campo `stfDecisionId` seja igual ao valor informado pelo usuário.
    ...
    2. Se o documento for encontrado, verificar se o campo `case_data.caseContent.md.doctrine` existe.
      1. Se o campo existir, imprimir o conteúdo do campo `case_data.caseContent.md.doctrine` na tela.
      2. Se o campo não existir, informar ao usuário que o campo `case_data.caseContent.md.doctrine` não foi encontrado.
         1. Concluir a execução do script.
3. Obter o valor do campo `case_data.caseContent.md.doctrines` do documento encontrado.
...
9. Salvar o documento JSON montado no campo `case_data.caseData.doctrineReferences` do documento correspondente ao stfDecisionId informado.
