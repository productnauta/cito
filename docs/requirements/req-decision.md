...
Utilize como referencia os scripts step07-extract-notes-mistral.py e step06-extract-legislation-mistral.py como referencia para a estrutura e arquitetura de código, crie um novo script py, que:
...
**Descrição**: O script deve iniciar carregando a partir dos arquivos YAML de **configuração**, os dados necessárias para conectar ao banco de dados MongoDB e para utilizar a API Mistral, e os prompts e parametros que serão utilziados.
    ...
    * Estruturar o código, para que seja facilmente definido qual provider de IA e prompt serão utilizados na etapa de consulta à API Mistral.
        - Deve ser possível definir o provider de IA e prompt a serem utilizados, alterando apenas uma variável no início do script, e as demais informações, parametros e configurações serão carregadas do respectivo itens dos arquivos de configuração.  
            ...
            - Exemplo: definindo no script o prompt_id = "extract-decision-details-stf", o sistema deverá carregar as respectivas configurações do prompt que serão utilizadas (apenas as que serão utilizadas) do arquivo prompts.yaml: role, content, template_variables, client_parameters, etc.   
...
u o prompt abaixo para realizar a extração das informações jurídicas de decisões do STF.
...
2. Consultar a collection case_data no banco de dados MongoDB, buscando o documento onde o campo `stfDecisionId` seja igual ao valor informado pelo usuário.
    ...
    2. Se o documento for encontrado, verificar se o campo `case_data.caseContent.md.decision` existe.
      1. Se o campo existir, imprimir o conteúdo do campo `case_data.caseContent.md.decision` na tela.
      2. Se o campo não existir, informar ao usuário que o campo `case_data.caseContent.md.decision` não foi encontrado na decisão.
         1. Concluir a execução do script.
...
4. Utilizando a API Mistral, enviar uma requisição para a API Mistral, utilizando os dadoos e parametros do prompt definido no início do script.
   1. Prompt definido: "get_decision-details-stf"
...
9. Salvar o documento JSON montado no campo `case_data.caseData.decisionDetails` do documento correspondente ao stfDecisionId informado.
10. Persistir logs e status da execução do script conforme o manual de persistência de status e execussões. 


## SCHEMA JSON DO DOCUMENTO case_data.caseData.decisionDetails
...
```json
{
  ...
  "decisionResult": {
    "finalDecision": "procedente|improcedente|parcialmente_procedente|outro|null"
  },
  "ministerVotes": [
    {
      "ministerName": "string",
      "voteType": "favoravel|contrario|divergente|vencido",
      "voteSummary": "string|null"
    }
  ],
  "partyRequests": [
    {
      "requestType": "liminar|merito|vista|outro",
      "requestingParty": "string|null",
      "requestDescription": "string",
      "requestOutcome": "atendido|nao_atendido|parcialmente_atendido|null"
    }
  ],
  "speakers": [
    {
  ...
  "citations": [
    {
      "citationType": "precedente_vinculante|precedente_persuasivo|doutrina|legislacao|outro",
      "citationName": "string",
  ...
  "decisionDates": {
    "judgmentStartDate": "YYYY-MM-DD|null",
    "judgmentEndDate": "YYYY-MM-DD|null"
  }
}