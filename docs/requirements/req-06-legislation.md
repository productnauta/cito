...
**Descrição**: O script deve iniciar carregando a partir dos arquivos YAML de **configuração**, os dados necessárias para conectar ao banco de dados MongoDB e para utilizar a API Mistral, e os prompts e parametros que serão utilziados.
    ...
    * Estruturar o código, para que seja facilmente definido qual provider de IA e prompt serão utilizados na etapa de consulta à API Mistral.
        - Deve ser possível definir o provider de IA e prompt a serem utilizados, alterando apenas uma variável no início do script, e as demais informações, parametros e configurações serão carregadas do respectivo itens dos arquivos de configuração.  
            ...
            - Exemplo: definindo no script o prompt_id = "extract-legislation-from-md", o sistema deverá carregar as respectivas configurações do prompt que serão utilizadas (apenas as que serão utilizadas) do arquivo prompts.yaml: role, content, template_variables, client_parameters, etc.   
        prompt_id = "get_legislation_from_stfDecisionId" 
...
2. Consultar a collection case_data no banco de dados MongoDB, buscando o documento onde o campo `stfDecisionId` seja igual ao valor informado pelo usuário.
    ...
    2. Se o documento for encontrado, verificar se o campo `case_data.caseContent.md.legislation` existe.
      1. Se o campo existir, imprimir o conteúdo do campo `case_data.caseContent.md.legislation` na tela.
      2. Se o campo não existir, informar ao usuário que o campo `case_data.caseContent.md.legislation` não foi encontrado na decisão.
         1. Concluir a execução do script.
3. Obter o valor do campo `case_data.caseContent.md` do documento encontrado.
4. Utilizando a API Mistral, enviar uma requisição para a API Mistral, utilizando os dadoos e parametros do prompt definido no início do script.
   1. Prompt definido: "get_legislation_from_stfDecisionId"
...
7. Lógica e regras para identificar e montar o documento JSON devem ser definidas conforme o schema do documento case_data.
   1. * **Formato**: 
        Linha `N|<id>|<jur>|<tipo>|<ano>|<desc>` 
        | Campo linha | Campo destino       | Normalização                                                 |
        | ----------- | ------------------- | ------------------------------------------------------------ |
        | `<id>`      | `normIdentifier`    | string (ex: `LEI-8112-1990`, `CF-1988`)                      |
        | `<jur>`     | `jurisdictionLevel` | `federal/state/municipal/unknown`                            |
        | `<tipo>`    | `normType`          | `CF/EC/LC/LEI/DECRETO/RESOLUCAO/PORTARIA/OUTRA`              |
        | `<ano>`     | `normYear`          | inteiro; se `0` → **ausente** (recomendo salvar `null`)      |
        | `<desc>`    | `normDescription`   | string; se vazio → ausente (recomendo salvar `""` ou omitir) |


        Linha: `R|<art>|<caput?>|<inc?>|<par?>|<par_unico?>|<letra?>`
        | Campo linha    | Campo destino       | Normalização                      |
        | -------------- | ------------------- | --------------------------------- |
        | `<art>`        | `articleNumber`     | int                               |
        | `<caput?>`     | `isCaput`           | `"1" => true`, vazio/`0` => false |
        | `<inc?>`       | `incisoNumber`      | int ou `null`                     |
        | `<par?>`       | `paragraphNumber`   | int ou `null`                     |
        | `<par_unico?>` | `isParagraphSingle` | `"1" => true`, vazio/`0` => false |
        | `<letra?>`     | `letterCode`        | string (ex: `"a"`) ou `null`      |
   ...
   2. * **Regras**:
        ...
        * `normIdentifier`:
          ...
          * `LEI-n-ano`
          * `DECRETO-n-ano`
          * remover ponto do número
        * `normYear`: inteiro; ausente `0` (no parsing: converter para `null`)
...
9. Salvar o documento JSON montado no campo `case_data.legislationReferences` do documento correspondente ao stfDecisionId informado.
10. Persistir logs e status da execução do script conforme o manual de persistência de status e execussões. 
